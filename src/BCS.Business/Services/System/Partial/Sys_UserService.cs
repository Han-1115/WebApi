    ...
    public partial class Sys_UserService
    {
        ...
        public async Task<WebResponseContent> Login(LoginInfo loginInfo, bool verificationCode = true)
        {
            ...
            try
            {
                ...
                var role = Sys_RoleRepository.Instance.FindFirstAsync(x => x.Role_Id == user.Role_Id).Result;

                string roleName = role?.RoleName ?? string.Empty;
                ...
                user.Token = token;
                webResponse.Data = new { token, userName = user.UserTrueName, img = user.HeadImageUrl, Employee_Number = user.Employee_Number, RoleNames = new List<string> { roleName } };
        ...
        /// <returns></returns>
        public Task<WebResponseContent> GetPagerList(PageDataOptions pageDataOptions)
        {
            ...
            var result = new PageGridData<Sys_User>
            {
                rows = currentPage,
                total = totalCount
            };

            return Task.FromResult(WebResponseContent.Instance.OK("查询用户信息成功", result));
        }
        ...
        /// <returns></returns>
        public Task<WebResponseContent> GetUsersByRole(string roleName)
        {
            List<string> roleNames = roleName.Split(',', StringSplitOptions.RemoveEmptyEntries).ToList();
            BCSContext context = DBServerProvider.GetEFDbContext();
            var userInfo = (from sysRole in context.Set<Sys_Role>()
                            where roleNames.Contains(sysRole.RoleName)
                            ...
                            select new
                            {
                                ...
                                sysUser.Email,
                                sysUser.Gender,
                                sysUser.HeadImageUrl,
                                sysUser.LastLoginDate,
                                sysUser.DeptIds,
                                sysUser.Employee_Number,
                            }).ToList();
            return Task.FromResult(WebResponseContent.Instance.OK("查询用户成功", userInfo));
        }
