 ...
 *Repository提供数据库操作，如果要增加数据库操作请在当前目录下Partial文件夹ProjectRepository编写代码
 */
...
namespace BCS.Business.Repositories
{
    ...
    {
    public ProjectRepository(BCSContext dbContext)
    : base(dbContext)
    {
    ...
    {
      get {  return AutofacContainerModule.GetService<IProjectRepository>(); } }
    }
}
