...
namespace BCS.System.Controllers
{
    public partial class Sys_WorkFlowController
    {
        ...
        {
            // TODO
            // 同一业务数据，多次提交工作流后，如果查询的问题。
            var flows = await _workFlowTableRepository.FindAsIQueryable(x => x.WorkTable == tableName && (x.AuditStatus != null && x.AuditStatus == (int)ApprovalStatus.PendingApprove) && ids.Contains(x.WorkTableKey))
            ...
            var steps = flow.Sys_WorkFlowTableStep
                    .Select(c => new
                    {
                        ...
                        c.Creator,
                        //判断是按角色审批 还是用户帐号审批
                        isCurrentUser = (c.AuditStatus == null || c.AuditStatus == (int)ApprovalStatus.PendingApprove)
                                        && c.StepId == flow.CurrentStepId && GetAuditStepValue(c),
                        isCurrent = c.StepId == flow.CurrentStepId && c.AuditStatus != (int)ApprovalStatus.Approved
                    }).OrderBy(o => o.OrderId);
