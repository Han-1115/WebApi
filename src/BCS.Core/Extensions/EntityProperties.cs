...
namespace BCS.Core.Extensions
{
    public static class EntityProperties
    {
        ...
        public static IQueryable<T> Contains<T>(this IQueryable<T> queryable, string field, string value)
        {
            if (value == null)
            {
                value = "";
            }
            return queryable.Where(field.CreateExpression<T>(value, LinqExpressionType.Contains));
        }
        public static IQueryable<T> LessOrequal<T>(this IQueryable<T> queryable, string field, string value)
        {
            if (value == null)
            {
                value = "";
            }
            return queryable.Where(field.CreateExpression<T>(value, LinqExpressionType.LessThanOrEqual));
        }
        public static IQueryable<T> ThanOrEqual<T>(this IQueryable<T> queryable, string field, string value)
        {
            if (value == null)
            {
                value = "";
            }
            return queryable.Where(field.CreateExpression<T>(value, LinqExpressionType.ThanOrEqual));
        }
        ...
        public static WebResponseContent ValidationEntity<T>(this T entity, string[] specificProperties, string[] validateProperties = null)
        {
            ...
            foreach (PropertyInfo propertyInfo in propertyArray)
            {
                ...
                //设置默认状态的值
                if (propertyInfo.Name == "Enable" || propertyInfo.Name == "approval_status")
                {
                    if (value == null)
                    {
        ...
        public static (bool, string, object) ValidationProperty(this PropertyInfo propertyInfo, object objectVal, bool required)
        {
            ...
            if ((required && val == string.Empty))
            {
                ...
                return (false, requiredMsg + (string.IsNullOrEmpty(propertyName) ? propertyInfo.Name : propertyName) + " cannot be empty", objectVal);
            }
            //列名
