    ...
    public static class AutofacContainerModuleExtension
    {
        ...
        public static IServiceCollection AddModule(this IServiceCollection services, ContainerBuilder builder, IConfiguration configuration)
        {
            ...
            if (DBType.Name == DbCurrentType.MySql.ToString())
            {
                ...
                services.AddDbContextPool<BCSContext>(optionsBuilder => { optionsBuilder.UseMySql(connectionString, new MySqlServerVersion(new Version(8, 0, 11))); }, 64);
            }
            else if (DBType.Name == DbCurrentType.PgSql.ToString())
            {
                services.AddDbContextPool<BCSContext>(optionsBuilder => { optionsBuilder.UseNpgsql(connectionString); }, 64);
            }
            else
            {
                services.AddDbContextPool<BCSContext>(optionsBuilder => { optionsBuilder.UseSqlServer(connectionString); }, 64);
            }
            //启用缓存
