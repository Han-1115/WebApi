...
namespace BCS.Core.Utilities
{
    public class EPPlusHelper
    {
            ...
            )
        {
            ...
            if (!file.Exists) return responseContent.Error("The uploaded file was not found. Please upload it again");
                ...
                for (int m = sheet.Dimension.Start.Row + 1, n = sheet.Dimension.End.Row; m <= n; m++)
                {
                    ...
                    for (int j = sheet.Dimension.Start.Column, k = sheet.Dimension.End.Column; j <= k; j++)
                    {
                        ...
                        if (string.IsNullOrEmpty(value))
                        {
                            if (options.Requierd)
                            {
                                return responseContent.Error($"The validation of column [{options.ColumnCNName}] in row {m} failed and cannot be empty");
                            }
                            continue;
                        }
                        ...
                        if (!string.IsNullOrEmpty(options.DropNo) && !string.IsNullOrEmpty(value)&&!ignoreSelectValidationFields.Contains(property.Name))
                        {
                            ...
                            {
                                return responseContent.Error($"The numerical dictionary number [{options.DropNo}] in field [{options.ColumnCNName}] is missing. Please check the dictionary configuration");
                            }
                            string key = null;
                            //2022.11.21增加导入多选的支持
                            ...
                            if (key == null)//&& options.Requierd
                            {
                                ...
                                return responseContent.Error($"The validation of column [{options.ColumnCNName}] in row {m} failed,must be the value of [{values}] in the dictionary data");
                            }
                        ...
                        if (!result.Item1)
                        {
                            return responseContent.Error($"The validation of column [{options.ColumnCNName}] in row {m} failed,{result.Item2}");
                        }
            ...
            using (ExcelPackage package = new ExcelPackage())
            {
                ...
                for (int i = 0; i < table.Columns.Count; i++)
                {
                    using (ExcelRange range = worksheet.Cells[1, i + 1])
                    {
                        ...
                        worksheet.Cells[1, i + 1].Style.Fill.BackgroundColor.SetColor(SixLabors.ImageSharp.Color.Gray);  //背景色
                        worksheet.Cells[1, i + 1].Style.Font.Color.SetColor(SixLabors.ImageSharp.Color.White);
                    }
            ...
            using (ExcelPackage package = new ExcelPackage())
            {
                ...
                for (int i = 0; i < propertyInfo.Count; i++)
                {
                    ...
                    using (ExcelRange range = worksheet.Cells[1, i + 1])
                    {
                        worksheet.Cells[1, i + 1].Style.Fill.PatternType = ExcelFillStyle.Solid;
                        //默认灰色背景，白色字体
                        SixLabors.ImageSharp.Color backgroundColor = SixLabors.ImageSharp.Color.Gray;
                        //字体颜色
                        SixLabors.ImageSharp.Color fontColor = SixLabors.ImageSharp.Color.White;
                        //下载模板并且是必填项，将表格设置为黄色
                        if (template)
                        {
                            fontColor = SixLabors.ImageSharp.Color.Black;
                            if (cellOptions.Exists(x => x.ColumnName == columnName && x.Requierd))
                            {
                                backgroundColor = SixLabors.ImageSharp.Color.Yellow;  //黄色必填
                            }
                            else
                            {
            ...
            using (ExcelPackage package = new ExcelPackage())
            {
                ...
                foreach (var item in rows[0])
                {
                    ...
                    style.Font.Bold = true;//字体为粗体
                                           //style
                    style.Fill.BackgroundColor.SetColor(SixLabors.ImageSharp.Color.FromRgb(216, 216, 216));//背景色
