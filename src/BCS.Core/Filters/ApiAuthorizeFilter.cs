...
using static BCS.Core.Filters.ApiTaskAttribute;
...
namespace BCS.Core.Filters
{
    public class ApiAuthorizeFilter : IAuthorizationFilter
    {
        ...
        public void OnAuthorization(AuthorizationFilterContext context)
        {
            ...
            var timespan = expDate - DateTime.Now;
            //动态标识刷新token(2021.05.01)
            if (timespan.TotalSeconds > 0 && timespan.TotalSeconds < AppSetting.ExpMinutes * 60 / 3 && context.HttpContext.Request.Path != replaceTokenPath)
            {
                context.HttpContext.Response.Headers.Add("bcs_exp", "1");
            }
