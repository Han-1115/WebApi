...
namespace BCS.Core.Filters
{
    public abstract class ServiceFunFilter<T> where T : class
    {
        ...
        /// <summary>
        /// 进入审批流程方法之前
        /// <para>1:更新扩展字段：BusinessName && BusinessType 字段</para>
        /// </summary>
        protected Action<T, Sys_WorkFlowTable> AddWorkFlowTableExecuting;
        ...
        /// bool:当前数据是否为最后一个人审核
        /// </summary>
        protected Func<T, ApprovalStatus, bool, WebResponseContent> AuditWorkFlowExecuting;

        /// <summary>
        /// 审批流程审核后
        /// T:当前审核的数据
        /// ApprovalStatus:审核状态
        /// filterOptions:当前审核步骤
        /// list:下一个节点的审批人id
        /// bool:当前数据是否为最后一个人审核
        /// </summary>
        protected Func<T, ApprovalStatus, FilterOptions, List<int>, bool, WebResponseContent> AuditWorkFlowExecuted;
