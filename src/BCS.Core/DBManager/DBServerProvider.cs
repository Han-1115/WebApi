...
namespace BCS.Core.DBManager
{
    public partial class DBServerProvider
    {
        ...
        public static BCSContext DbContext
        {
            get { return GetEFDbContext(); }
        }
        public static BCSContext GetEFDbContext()
        {
            return GetEFDbContext(null);
        }
        public static BCSContext GetEFDbContext(string dbName)
        {
            BCSContext context = Utilities.HttpContext.Current.RequestServices.GetService(typeof(BCSContext)) as BCSContext;
            if (dbName != null)
            {
        ...
        public static void SetDbContextConnection(BCSContext beefContext, string dbName)
        {
            if (!ConnectionPool.ContainsKey(dbName))
            {
                throw new Exception("数据库连接名称错误");
        ...
        /// <returns></returns>
        public static void GetDbContextConnection<TEntity>(BCSContext defaultDbContext)
        {
