CREATE TABLE [dbo].[SubContractFlow] (
    [Id]                                  INT               IDENTITY (1, 1) NOT NULL,---主键ID
    [SubContract_Id]                      INT               NOT NULL DEFAULT (0),  ---分包合同ID
    ...
    [Project_Billing_Mode]                NVARCHAR(50) NOT NULL DEFAULT (''), --项目结算模式
    [Subcontracting_Cost]                 DECIMAL(20,2) NOT NULL DEFAULT (0) ,--分包成本（含税）,
    [Subcontracting_Balance]              DECIMAL(20,2) NOT NULL DEFAULT (0) ,--分包成本（余额）,
    [Contract_Code]                       NVARCHAR (50)   NULL DEFAULT (''),---合同注册编码（Contract Registration ID
    [Contract_Name]                       NVARCHAR (500)   NULL DEFAULT (''),---合同名称
    [Client_Organization_Name]            NVARCHAR (500)   NULL DEFAULT (''),---客户名称
    ...
    [Subcontracting_Reason]               NVARCHAR(500)    NOT NULL DEFAULT (''), --分包原因
    ...
    [Type]                                INT               NULL,---类型：1注册，2变更, 3分包人员修改
    ...
    CONSTRAINT [PK_SubContractFlow] PRIMARY KEY CLUSTERED ([Id] ASC)
);
